...
- 精确加载预设的新工作流：先点"清空"，再点"应用"（由替换语义变为合并语义后的等效替代）
...
| 搜索筛选状态下应用 | 匹配全量 `sourceGroups`（非仅可见），可能往选中追加当前不可见的 CID。与改动前行为一致——`applySavedList` 始终匹配全量来源箱 |

## 测试

`src/inject/StockMovePanel.test.js` 新增：

1. **合并模式** — 手动勾选部分分组 → 应用已保存列表 → 断言选中 = 原有 ∪ 预设（去重）
2. **无匹配回归** — 先勾选部分分组 → 应用无匹配预设 → 断言显示错误但已有选中不变
