...
describe('trade-info-history-recorder', () => {
  it('writes Cids.json with unique positive cids', () => {
    ...
    try {
      ...
      expect(JSON.parse(fs.readFileSync(outputPath, 'utf8'))).toEqual([1032006, 1013007]);
    } finally {
  ...
  it('skips non-convertible collection cids without throwing', () => {
    ...
    try {
      ...
      const outputPath = path.join(rootDir, 'Cids.json');
      expect(result).toEqual({ written: true, itemCids: [1032006], outputPath });
      expect(JSON.parse(fs.readFileSync(outputPath, 'utf8'))).toEqual([1032006]);
    } finally {
      fs.rmSync(rootDir, { recursive: true, force: true });
    }
  });
