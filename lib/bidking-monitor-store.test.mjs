...
describe('BidKing monitor store', () => {
  ...
  it('keeps Elsa complete-reveal groups at zero when the target quality has no outlines', () => {
    const state = applyBidKingMonitorFacts(
      createEmptyBidKingMonitorState(ELSA_MONITOR_PROFILE),
      [
        { type: 'group.completeReveal', key: 'elsa-white-empty', gameUid: 'game-1', group: 'white' },
      ],
      ELSA_MONITOR_PROFILE,
    );

    expect(state.profileId).toBe('elsa');
    expect(state.completeRevealGroups).toEqual(['white']);
    expect(state.groups.white.totalCells).toBe(0);
  });
});
