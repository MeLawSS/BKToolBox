...
class BidKingLiveMonitor extends EventEmitter {
  ...
  async start(options = {}) {
    ...
    bidkingMonitorGrid.setInferenceAlgorithmV2(this.options.useInferenceV2);
  ...
  publicOptions(options = this.options) {
    ...
    return {
      ...
      outputDir: options.outputDir,
      useInferenceV2: options.useInferenceV2,
    };
  }
}
...
function normalizeOptions(options = {}, runtimeRoot = getRuntimeRoot(), fallbackOutputDir) {
  ...
  const useInferenceV2 = options.useInferenceV2 === true || options.useInferenceV2 === 'true';
