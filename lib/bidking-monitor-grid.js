...
function inferMinimumOccupiedCellsV2({ outlines = [], columns = MONITOR_GRID_COLUMNS } = {}) {
  ...
  // Seed occupied with all known outline cells
  ...
  // Default prefix: cells before the anchor outline that must be occupied
  ...
  // Ensure each outline is blocked from being placed before its boxId
  for (const outline of sorted) {
    const blocked = ensureBlocked(occupied, unknownBlockers, outline, sorted, columns, 0);
    if (!blocked) {
 ...
 */
function ensureBlocked(occupied, unknownBlockers, outline, allOutlines, columns, depth) {
  ...
  while (true) {
    ...
    for (const candidate of candidates) {
      ...
      if (blockerCells.some((c) => occupied.has(c))) continue;
      ...
      if (ensureBlocked(occupied, unknownBlockers, blockerOutline, allOutlines, columns, depth + 1)) {
        placed = true;
        break;
      }
...
function enumerateBlockerCandidates(occupied, threatPos, threatW, threatH, maxPos, columns) {
  ...
  // Sort by:
  //  1. Full threat coverage preferred (prevents cascading gaps)
  //  2. Efficiency (threat cells blocked per new cell)
  //  3. Fewer new cells
  //  4. Larger position (closer to target)
  const threatSize = threatFootprint.length;
  candidates.sort((a, b) => {
    const aFull = a.threatCellsBlocked >= threatSize ? 0 : 1;
    const bFull = b.threatCellsBlocked >= threatSize ? 0 : 1;
    return aFull - bFull
      || (b.threatCellsBlocked / b.newCellCount) - (a.threatCellsBlocked / a.newCellCount)
