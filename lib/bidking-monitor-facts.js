...
function getBoxQuality(box, profile) {
  ...
  const name = rawName === undefined || rawName === null ? null : String(rawName);
  return {
    id,
...
function buildAggregateFact(event, source, profile) {
  ...
  if (isTotalCellsSkillName(name)) {
    return buildGroupFact(event, source, 'group.totalCellsKnown', group, skill.totalHitBoxIndex);
  }
  if (isAverageCellsSkillName(name)) {
    return buildGroupFact(event, source, 'group.averageCellsKnown', group, skill.allHitItemAvgBoxIndex);
  }
